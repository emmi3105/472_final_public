...
The GitHub repository for **this assignment** can be found [here](https://github.com/emmi3105/472_final_public).^[Note that, in our RMarkdown, we disabled the execution of the chunks containing code for automatic webscraping and sending requests to our APIs with eval=False as to prevent retrieving the data over and over again.] Please refer to the document called `running_code_locally.Rmd` available in the repository and follow the instructions to run the code on your local machine.
...
The RS-T100, which exhibit outlier values for endurance scores, may not be confined solely to the Rolling Stones ranking, but could also be found in the Spotify-T50 and LastFM-T50. Therefore, we look at the intersections of the RS-T100, Spotify-T50 and LastFM-T50.
...
venn_riaa_albums <- venn.diagram(
  ...
  category.names = c("Rolling Stone's Top 100" , "RIAA albums"),
